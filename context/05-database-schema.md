...
| Feature | Usage |
|---|---|
| `auth.users` | Base user records, phone OTP, JWT issuance |
| Row Level Security (RLS) | All tables — role-based data access |
| Realtime Subscriptions | `mechanic_locations`, `messages`, `jobs` |
| Storage Buckets | `avatars`, `documents`, `job-evidence`, `parts-images` |
| Edge Functions | M-Pesa callback handler, payout trigger, cron jobs |
| `pg_cron` | Auto-cancel stale PENDING jobs, escrow release after 15min |
...
CREATE TYPE job_status AS ENUM (
  'DRAFT', 'PENDING', 'MATCHED', 'ACCEPTED',
...
CREATE TYPE service_category AS ENUM (
  ...
  'brake_repair', 'electrical', 'ac_repair',
  'body_repair', 'diagnosis', 'general', 'other'
);

CREATE TYPE payment_method AS ENUM ('mpesa', 'card', 'wallet');

CREATE TYPE payment_status AS ENUM (
  'PENDING', 'ESCROWED', 'SETTLED', 'FAILED', 'REFUNDED', 'PARTIAL_REFUND'
);

CREATE TYPE payment_reference_type AS ENUM ('job', 'parts_order', 'wallet_topup');

CREATE TYPE order_status AS ENUM (
  'PENDING_PAYMENT', 'CONFIRMED', 'PACKED', 'DISPATCHED', 'DELIVERED', 'CANCELLED'
);
...
CREATE TYPE notification_type AS ENUM (
  'job_created', 'job_accepted', 'mechanic_en_route', 'mechanic_arrived',
  'job_completed', 'payment_received', 'payout_sent', 'dispute_raised',
  'dispute_resolved', 'order_dispatched', 'order_delivered', 'kyc_update'
);
```
...
CREATE TABLE profiles (
  ...
  phone         TEXT UNIQUE NOT NULL,        -- mirrors auth.users phone
  email         TEXT,
  avatar_url    TEXT,
...
CREATE TABLE mechanic_profiles (
  ...
  mpesa_number        TEXT,                          -- for B2C payouts
  bank_account        JSONB,                         -- { bank, account_no, account_name }
...
CREATE POLICY "Customers read approved mechanics" ON mechanic_profiles
  FOR SELECT USING (kyc_status = 'APPROVED');
CREATE POLICY "Admins full access" ON mechanic_profiles
  USING ((SELECT role FROM profiles WHERE id = auth.uid()) IN ('admin', 'dispatcher'));
```
...
CREATE TABLE mechanic_documents (
  ...
  doc_type      TEXT NOT NULL,    -- national_id_front | national_id_back | selfie | certificate
...
CREATE POLICY "Mechanic reads own docs" ON mechanic_documents FOR SELECT USING (
  auth.uid() = mechanic_id
);
CREATE POLICY "Admins full access" ON mechanic_documents
  USING ((SELECT role FROM profiles WHERE id = auth.uid()) IN ('admin', 'dispatcher'));
```
...
### `mechanic_locations`
High-write ephemeral table. Upsert only.
```sql
...
CREATE POLICY "Mechanics update own location" ON mechanic_locations
  FOR ALL USING (auth.uid() = mechanic_id);
CREATE POLICY "Customers read location for active job" ON mechanic_locations
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM jobs
...
CREATE TABLE jobs (
  ...
  evidence_urls     TEXT[] DEFAULT '{}',     -- mechanic completion evidence

  -- Location (snapshot at job creation)
  location_lat      NUMERIC(10,7) NOT NULL,
  location_lng      NUMERIC(10,7) NOT NULL,
  location_address  TEXT NOT NULL,

  -- Pricing
  price_estimate_min   INTEGER,            -- KES
  price_estimate_max   INTEGER,
  agreed_price         INTEGER,            -- set on acceptance
  currency             TEXT NOT NULL DEFAULT 'KES',
  payment_status       payment_status NOT NULL DEFAULT 'PENDING',

  -- Scheduling
  scheduled_at         TIMESTAMPTZ,        -- null for emergency
...
### `job_bids` (marketplace / non-emergency)
```sql
...
CREATE POLICY "Mechanic manages own bids" ON job_bids
  FOR ALL USING (auth.uid() = mechanic_id);
CREATE POLICY "Customer reads bids on own jobs" ON job_bids
  FOR SELECT USING (
...
CREATE TABLE transactions (
  ...
  reference_id       UUID NOT NULL,          -- job.id or parts_order.id
  payer_id           UUID NOT NULL REFERENCES profiles(id),
  payee_id           UUID REFERENCES profiles(id),    -- null if platform collection
...
CREATE TABLE parts (
  ...
  compatible_with JSONB DEFAULT '[]',    -- [{ make, model, year_from, year_to }]
...
CREATE POLICY "Public can read active parts" ON parts FOR SELECT USING (is_active = true);
CREATE POLICY "Supplier manages own parts" ON parts
...
CREATE TABLE parts_orders (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  customer_id      UUID NOT NULL REFERENCES profiles(id),
  job_id           UUID REFERENCES jobs(id),    -- optional: linked to a repair job
...
CREATE POLICY "Customer reads own orders" ON parts_orders
...
CREATE POLICY "Order item read through order" ON parts_order_items
  FOR SELECT USING (
...
CREATE POLICY "Job parties read messages" ON messages
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM jobs
      WHERE id = job_id
...
CREATE POLICY "Job parties send messages" ON messages
...
CREATE TABLE reviews (
  ...
  rated_role     TEXT NOT NULL,   -- 'mechanic' | 'customer'
...
CREATE POLICY "Job parties create reviews" ON reviews
  FOR INSERT WITH CHECK (
...
CREATE POLICY "Public read mechanic reviews" ON reviews
  FOR SELECT USING (rated_role = 'mechanic');
```
...
CREATE POLICY "Parties read own disputes" ON disputes
  FOR SELECT USING (
    ...
    EXISTS (SELECT 1 FROM jobs WHERE id = job_id AND mechanic_id = auth.uid())
  );
CREATE POLICY "Admins full access" ON disputes
  USING ((SELECT role FROM profiles WHERE id = auth.uid()) IN ('admin', 'dispatcher'));
```
...
CREATE TABLE notifications (
  ...
  title           TEXT NOT NULL,
  body            TEXT NOT NULL,
  data            JSONB DEFAULT '{}',    -- deep-link or reference payload
...
CREATE POLICY "User marks own as read" ON notifications
...
CREATE TRIGGER trg_mechanic_profiles_updated BEFORE UPDATE ON mechanic_profiles
  FOR EACH ROW EXECUTE FUNCTION touch_updated_at();
CREATE TRIGGER trg_jobs_updated BEFORE UPDATE ON jobs
  FOR EACH ROW EXECUTE FUNCTION touch_updated_at();
-- (repeat for parts, parts_orders, disputes)

-- Update mechanic rating on new review
...
-- Auto-cancel PENDING jobs older than 10 min (via pg_cron or Edge Function)
-- SELECT cron.schedule('cancel-stale-jobs', '* * * * *', $$
--   UPDATE jobs SET status = 'CANCELLED' WHERE status = 'PENDING' AND created_at < now() - INTERVAL '10 minutes';
-- $$);
```
...
|---|---|---|---|
| `avatars` | Public read, auth write | 5MB | image/* |
| `documents` | Private (owner + admin) | 10MB | image/*, application/pdf |
| `job-evidence` | Private (job parties + admin) | 20MB | image/* |
| `parts-images` | Public read, supplier write | 10MB | image/* |
...
|---|---|
| `profiles` | phone, role |
| `mechanic_profiles` | kyc_status, is_online, specialisations (GIN), rating_avg |
| `mechanic_locations` | geom (GIST) — for ST_DWithin radius queries |
| `jobs` | customer_id, mechanic_id, status, created_at, location (GIST) |
| `transactions` | payer_id, (reference_type, reference_id), status, mpesa_ref |
| `messages` | (job_id, sent_at) |
| `reviews` | reviewee_id |
| `parts` | supplier_id, category, text search (GIN) |
| `notifications` | (user_id, created_at DESC), is_read filter |
